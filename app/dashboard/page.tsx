...
export default async function Dashboard() {
    ...
    const lowStockAtCount = allProduct.filter((p) => Number(p.quantity) <= 5 && Number(p.quantity) >= 1).length;
    const outOfStockCount = allProduct.filter((p) => Number(p.quantity) === 0).length;

    const inStockPercentage = totalProduct > 0 ? Math.round((inStockCount / totalProduct) * 100) : 0;
    const lowStockPercentage = totalProduct > 0 ? Math.round((lowStockAtCount / totalProduct) * 100) : 0;
    const outOfStockPercentage = totalProduct > 0 ? Math.round((outOfStockCount / totalProduct) * 100) : 0;
